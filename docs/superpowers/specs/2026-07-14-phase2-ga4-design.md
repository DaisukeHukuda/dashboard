...
- ステータス: **確定（plan 化可能）**。GA4プロパティID・対象サイト・認証主体すべて確定（§9）。
...
1. **GA4 計測**：回っている（✅）。ただし **GA4 プロパティID（数値）は未取得**＝実装接続時に必要。**GA4 が入っているサイトが何か**（自社サイト/ブログ/LP等）も要確認＝"認知"の測定対象を確定するため。
2. **認証主体**：**既存SA `urakata-sync@urakata-calendar.iam.gserviceaccount.com` を再利用に決定（✅）**。ユーザー作業＝GA4管理画面でこのSAメールを「閲覧者」に追加（1回）。新規SA作成・鍵JSON発行は不要。
3. **コンバージョンイベント**：**未設定**（✅回答）。→ GA4上の「予約完了」CVは使わない。**「GA4の日次セッション（認知）× `history:latest` の日次予約（成果）」を同一タイムラインで重ねる**方式を採用（§4.5）。ファネルCVカードは Phase 2 では作らない。
4. **予約経路**：**アソビューの「ウラカタ」1つに集約**（✅）。→ **予約完了はアソビュー（外部ドメイン）で発生し、ユーザーのGA4では直接計測できない**。よって GA4 は「予約前の認知・検討トラフィック（自社サイト/流入元）」を測り、実際の予約は Phase 1 の `history:latest` が担う。両者を重ねて「流入が予約に効いているか」を見る（§4.5）のが本Phaseの中核。可能なら**アソビューへの外部リンククリック**をGA4イベント化できると導線が明確になる（任意・将来）。

### 確定値（2026-07-14 すべて確定 → plan 化可能）
- (a) **GA4 プロパティID = `312598868`**
- (b) **対象サイト = `https://www.supsup.jp`**（自社の認知・検討サイト）。予約完了は別ドメインのアソビュー `supsup.urkt.in` で、GA4では測れない → §4.5 の「supsup.jp 流入 × 予約実績」の重ね描きが導線把握の中核。
- (c) **認証主体 = 既存SA `urakata-sync@urakata-calendar.iam.gserviceaccount.com` を再利用**。

**本番接続に必要なユーザー作業（実装後）**:
1. GA4 管理 → プロパティ「アクセス管理」で、上記SAメールを**「閲覧者」**に追加。
2. dashboard の Worker Secret に `GA4_SA_JSON_B64`（SA鍵JSONのbase64）と `GA4_PROPERTY_ID=312598868` を設定。
   - SA鍵JSONは GCP のサービスアカウント → キー で発行（既存の鍵を流用できるなら流用可）。
（コード骨組み＝`ga4/auth`・`ga4/client`・`ga4/reports`・`metrics/traffic` は上記作業前でも TDD＋モックで実装・検証可能。）

> 実装コードの骨組み（`ga4/auth`・`ga4/client`・`metrics/traffic` の TDD 雛形）は上記確定前でも先行可能。§4 のうち「認知→予約の重ね描き(§4.5)」「流入チャネル/source-medium(§4.1-4.2)」を主軸、CV/ファネルは対象外に変更。
