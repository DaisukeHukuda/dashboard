...
> - ✅ **Step 5 完了（2026-07-18）**: Instagram接続済み。Metaアプリ `supsup-inst`（ID 1050110217698332・ユースケース「Instagramでメッセージとコンテンツを管理」のFacebookログイン方式・4権限テスト準備完了）で長期トークン発行→交換（**2026-09-05 に更新済み→次回失効 2026-11-04 頃**）。IGユーザーID `17841403433521466`（@supsupnikko・フォロワー3788）。疎通テスト（アカウント/insights/media）合格→ Secrets `IG_ACCESS_TOKEN`/`IG_USER_ID` 登録→デプロイ（Version 31c4bf38）。※トークン再発行時は同アプリのグラフAPIエクスプローラで再生成→交換→Secret更新
...
- **Step 2 完了後なら**：KPI・トレンド・ヒートマップ・コホート・コース別に**実データ**が出る
...
**確認**: ダッシュボードで **Webサイトビュー（`?view=web` またはサイドバーの「Webサイト」）** を開く
...
**確認**: ダッシュボードで **Instagramビュー（`?view=sns`）** を開く
- 「Instagram（SNS）」のカード群が表示される（フォロワー推移・リーチ・投稿別エンゲージメント・投稿×予約）
- フォロワー推移は 2026-09-06 以降、**毎日 JST 01:00 に Cron Trigger で自動記録**（Cron は `wrangler deploy` で自動登録され、追加手順なし。Cloudflare ダッシュボード → Workers → 該当 Worker → Settings → Triggers で確認可）。それ以前は閲覧日のみの記録なので点がまばら。
...
失効チェックは **Instagram ビュー**を開いて確認する（既定の予約分析ビューには表示されない）。
...
- [x] Step 5: トークン更新 1回目実施（2026-09-05・疎通OK）。**次回失効 2026-11-04 頃**→10月下旬に再発行（手順: グラフAPIエクスプローラで再生成→ig-setup.txtのTOKEN差し替え→Claudeが交換・`wrangler secret put IG_ACCESS_TOKEN`）
...
| フォロワー推移が空 | **仕様**（今日から蓄積・過去は遡れない。記録は毎日 JST 01:00 の Cron（閲覧時も未記録なら1回記録）） |
| GA4カードは出るが前期比が出ない | 前期データの取得失敗（一時的・当期表示には影響なし）。再読み込みで回復することが多い |
| フォロワー推移が途中で止まった／横ばいのまま増えない | IGトークン失効の可能性（次回失効 2026-11-04 頃）。Instagram ビューでトークン状態を確認し、更新する |
